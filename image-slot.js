...
(() => {
  // Legacy single-file sidecar (still READ so previously-saved images show).
  const LEGACY_FILE = '.image-slots.state.json';
  // Per-page sidecar. One shared file for the whole site overflows the host's
  // write ceiling (~2MB) and then silently drops every later drop, so each
  // page saves only its own slots. Basename must end in .state.json for the
  // host write allowlist.
  const PAGE_SLUG = ((location.pathname.split('/').pop() || 'index')
    .replace(/\.dc\.html$|\.html$/i, '').toLowerCase().replace(/[^a-z0-9_-]/g, '-')) || 'index';
  // The host write-allowlist accepts exactly one lowercase [a-z0-9_-] segment
  // before ".state.json" (an interior dot is rejected), so the whole filename
  // must be a single dot-free, lowercase segment.
  const STATE_FILE = 'imgslots-' + PAGE_SLUG + '.state.json';
  // ids belonging to slots mounted on THIS page — only these get written to
  // the page sidecar, so it stays small no matter how big the whole site is.
  const owned = new Set();
  // Cap the longest side at 1500px — plenty sharp for these square tiles and
  // gallery layouts, and small enough (~150KB) that a full page of photos
  // saves reliably under the write ceiling.
  const MAX_DIM = 3000;
  ...
  function load() {
    ...
    loadP = Promise.all([grab(LEGACY_FILE), grab(STATE_FILE)])
      .then(([legacy, page]) => {
        const j = Object.assign({}, legacy || {}, page || {});
  ...
  function save() {
    ...
    if (!w) return;
    saving = true;
    // Only this page's own slots go into its sidecar, keeping it small.
    const out = {};
    for (const id of owned) if (slots[id] != null) out[id] = slots[id];
